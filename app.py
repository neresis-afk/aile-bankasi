...
.login-box {
    ...
    margin: 2rem auto; max-width: 440px;
}
.login-box h2 { font-family: 'Baloo 2', cursive; color: #fff; font-size: 1.6rem; margin-bottom: 0.4rem; }
.login-box p  { color: #a0b4cc; font-size: 0.95rem; line-height: 1.6; margin-bottom: 1.2rem; }
.login-icon   { font-size: 3.5rem; display: block; margin-bottom: 0.6rem; }
.auth-url {
...
.denied-box h2 { font-family: 'Baloo 2', cursive; color: #f87171; font-size: 1.6rem; margin-bottom: 0.4rem; }
.denied-box p  { color: #a0b4cc; font-size: 0.95rem; line-height: 1.6; }
.user-badge {
...
# ── Constants ─────────────────────────────────────────────────────────────────
# Sheets access scope + email read scope (to verify identity)
SCOPES = [
...
def get_allowed_emails() -> list[str]:
    """Load the whitelist from secrets (cloud) or allowed_emails.txt (local)."""
    try:
        emails = st.secrets["allowed_emails"]
    ...
    if os.path.exists("allowed_emails.txt"):
        return [
            line.strip().lower()
            for line in open("allowed_emails.txt").readlines()
            if line.strip()
        ]
    return []
...
def get_user_email(creds: Credentials) -> str:
    """Fetch the signed-in user's email via Google userinfo endpoint."""
...
def finish_oauth_flow(code: str) -> Credentials | None:
    ...
    try:
        ...
        open(TOKEN_FILE, "w").write(creds.to_json())
        print("\n✅ TOKEN JSON (Streamlit Cloud secrets için):\n")
        print(creds.to_json())
        return creds
...
# ── Resolve credentials for current environment ───────────────────────────────
def resolve_credentials() -> Credentials | None:
    """
    Cloud  → always use the token baked into secrets (app's own token for Sheets).
    Local  → load from token.json or run interactive OAuth flow.
    Returns None if local flow is incomplete (user hasn't pasted code yet).
    """
    if is_cloud():
        return get_cloud_credentials()
    return load_local_credentials()   # may be None → triggers login UI below

# ══════════════════════════════════════════════════════════════════════════════
# LOCAL LOGIN UI  (only shown when running locally without a valid token)
# ══════════════════════════════════════════════════════════════════════════════
creds = resolve_credentials()

if not is_cloud() and creds is None:
    ...
    with col2:
        ...
        <div class="login-box">
          ...
          <p>Aşağıdaki bağlantıyı açın, Google hesabınızla
             giriş yapın ve size verilen kodu buraya yapıştırın.</p>
          <div class="auth-url">{auth_url}</div>
        </div>
        """, unsafe_allow_html=True)
        st.link_button("🌐 Google Yetkilendirme Sayfasını Aç", auth_url)
        code = st.text_input("Google'dan aldığınız kod:", placeholder="4/0AX...")
...
# ══════════════════════════════════════════════════════════════════════════════
def get_verified_email(creds: Credentials) -> str:
    """
    Cache the verified email in session_state so we don't call the API
    on every Streamlit rerun.
    """
    if "verified_email" not in st.session_state:
        try:
            st.session_state["verified_email"] = get_user_email(creds)
        except Exception:
            st.session_state["verified_email"] = ""
    return st.session_state["verified_email"]

allowed_emails = get_allowed_emails()
user_email     = get_verified_email(creds)
    ...
    with col2:
        ...
        <div class="denied-box">
          ...
          <p>Bu uygulama yalnızca aile üyelerine özeldir.<br>
             <b>{user_email}</b> hesabının erişim izni bulunmuyor.</p>
        </div>
        """, unsafe_allow_html=True)
        if st.button("🔄 Farklı Hesapla Giriş Yap"):
            # Clear token so login flow restarts
...
@st.cache_resource
def get_gc_cloud():
    return gspread.authorize(get_cloud_credentials())

@st.cache_resource
def get_gc_local(token_sig: str):
    creds = Credentials.from_authorized_user_file(TOKEN_FILE, SCOPES)
...
def get_gc():
    if is_cloud():
        return get_gc_cloud()
    sig = open(TOKEN_FILE).read()[:40] if os.path.exists(TOKEN_FILE) else ""
    return get_gc_local(sig)
