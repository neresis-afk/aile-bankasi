...
def get_client_config() -> dict:
    """
    Cloud  -> load OAuth client config from st.secrets["client_secret"]
    Local  -> load from client_secret.json file
    """
    if is_cloud():
        s = st.secrets["client_secret"]
        return {
            "installed": {
...
def start_login_flow() -> str:
    flow = InstalledAppFlow.from_client_config(
        get_client_config(), scopes=LOGIN_SCOPES,
...
if not visitor_creds:
    if not is_cloud() and not os.path.exists("client_secret.json"):
